...
- **H.264 status — BLOCKED on SPS/PPS (catch-22), MJPEG is the shipping path.** Detailed RE findings (CedarC v1.2.0):
  ...
  - **Recommended next step**: dynamic analysis — `gdb`/`ltrace` the chroot encoder to find the exact SetParameter/state sequence that flips `ctx[966]` (or populates `ctx[5664]`) WITHOUT the `0x106` HW hang; or capture AVA's own working SPS/PPS. Tools ready: `cedar_enc.c` (encode + `0x501`/SetParameter probes via `H264IDX`/`H264PROF` env), `h264_headers.py`, vendor libs at `~/dreame-re/venc/`.
